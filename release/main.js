(function (_, Kotlin, $module$screeps_kotlin_types_jsLegacy) {
  ...
  function build($receiver, assignedRoom) {
    ...
    var tmp$ = get_building($receiver.memory);
    if (tmp$) {
      tmp$ = $receiver.store[RESOURCE_ENERGY] === 0;
    }if (tmp$) {
      set_building($receiver.memory, false);
      $receiver.say('\uD83D\uDD04 harvest');
    }var tmp$_0 = !get_building($receiver.memory);
    if (tmp$_0) {
      tmp$_0 = $receiver.store[RESOURCE_ENERGY] == $receiver.store.getCapacity();
    }if (tmp$_0) {
      set_building($receiver.memory, true);
      $receiver.say('\uD83D\uDEA7 build');
    }if (get_building($receiver.memory)) {
      var targets = assignedRoom.find(FIND_MY_CONSTRUCTION_SITES);
      if (!(targets.length === 0)) {
        if (equals($receiver.build(targets[get_resourceIndex($receiver.memory)]), ERR_NOT_IN_RANGE)) {
          $receiver.moveTo(targets[get_resourceIndex($receiver.memory)].pos);
        }}} else {
  ...
  function harvest($receiver, fromRoom, toRoom) {
    ...
    if (compareTo($receiver.store[RESOURCE_ENERGY], $receiver.store.getCapacity()) < 0) {
      ...
      if (equals($receiver.harvest(sources[get_resourceIndex($receiver.memory)]), ERR_NOT_IN_RANGE)) {
        var error = $receiver.moveTo(sources[get_resourceIndex($receiver.memory)].pos);
        if (equals(error, OK))
      ...
      if (equals($receiver.transfer(target, RESOURCE_ENERGY), ERR_NOT_IN_RANGE)) {
        console.log('storing...');
        var error_0 = $receiver.moveTo(target.pos);
        if (!equals(error_0, OK))
          console.log($receiver.name + ' is unable to move due to: ' + error_0);
      }}
  }
  ...
  function doTemporaryTask($receiver) {
    ...
    }$receiver.say('temp: ' + get_temporaryTask($receiver.memory));
  ...
  function determineBestWorkerBody(spawn) {
    ...
    var $receiver_0 = until(0, nrOfParts);
    var destination = ArrayList_init(collectionSizeOrDefault($receiver_0, 10));
    var tmp$_0;
    tmp$_0 = $receiver_0.iterator();
    while (tmp$_0.hasNext()) {
      var item = tmp$_0.next();
      destination.add_11rb$(WORK);
    }
  ...
  function spawnCreeps(creeps, spawn_0) {
    ...
    loop: for (var i = 0; i !== tmp$.length; ++i) {
      var tmp$_0;
      if (shouldBuildMoreWorkers(get_buildersNeeded(roomMemory).get_za3lpa$(i), creeps, Role$BUILDER_getInstance(), i))
        tmp$_0 = Role$BUILDER_getInstance();
      else if (shouldBuildMoreWorkers(get_harvestersNeeded(roomMemory).get_za3lpa$(i), creeps, Role$HARVESTER_getInstance(), i))
  ...
  function spawn(spawn, body, role, resource) {
    var newName = toString(Kotlin.getKClassFromExpression(role).simpleName) + '_' + Game.time;
  ...
  function gameLoop() {
    ...
    tmp$ = firstOrNull_0(Object.values($receiver));
    if (tmp$ == null) {
  ...
  package$building.buildRoadsToSources = buildRoadsToSources;
  ...
  package$creeps.build_hrmv4t$ = build;
